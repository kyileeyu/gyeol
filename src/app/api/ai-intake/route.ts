    ...
    <table role="presentation" style="max-width:560px;margin:0 auto;background:${C.card};border-radius:12px;overflow:hidden;border:1px solid ${C.border};">
      <tr>
        <td style="padding:24px 28px;border-bottom:1px solid ${C.border};">
          <p style="margin:0;color:${C.muted};font-size:11px;letter-spacing:0.18em;text-transform:uppercase;">[결 컨설팅] 사전 문의</p>
  ...
  const { error } = await resend.batch.send([
    {
      ...
      to,
      replyTo: email,
      subject: `[결 컨설팅] 사전 문의 — ${name} / ${role} / ${blockerKo}`,
      html: adminHtml,
      text: `[결 컨설팅] 사전 문의\n이름: ${name}\n이메일: ${email}\n직무·업종: ${role}\n막히는 자리: ${blockerKo}\n\n풀고 싶은 결정/업무:\n${decision}`,
    },
    {
