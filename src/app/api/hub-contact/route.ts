// /me 소개 허브 — 가벼운 문의 API Route Handler (기존 /api/contact 불가촉)
import { NextResponse } from "next/server";
import { Resend } from "resend";
import { hubContactSchema } from "@/features/intro-hub/lib/hub-contact-schema";
...
export async function POST(req: Request) {
  ...
  const parsed = hubContactSchema.safeParse(payload);
  if (!parsed.success) {
  ...
  if (!apiKey || !to || !from) {
    console.error("[hub-contact] missing env vars", {
      hasKey: !!apiKey,
      hasTo: !!to,
      hasFrom: !!from,
    });
    ...
    <table role="presentation" style="max-width:560px;margin:0 auto;background:${C.card};border-radius:16px;overflow:hidden;border:1px solid ${C.border};">
      <tr>
        <td style="padding:24px 28px;border-bottom:1px solid ${C.border};">
          <p style="margin:0;color:${C.muted};font-size:11px;letter-spacing:0.12em;text-transform:uppercase;">[허브 문의] /me</p>
          <h1 style="margin:6px 0 0;font-size:18px;color:${C.deep};font-weight:500;">${escape(name)}</h1>
        </td>
      </tr>
      <tr>
...
<html lang="ko">
  <body style="margin:0;padding:24px;background:${C.bg};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
    <table role="presentation" style="max-width:560px;margin:0 auto;background:${C.card};border-radius:16px;overflow:hidden;border:1px solid ${C.border};">
      <tr><td style="padding:32px 32px 8px;"><p style="margin:0;color:${C.muted};font-size:11px;letter-spacing:0.16em;text-transform:uppercase;">결 · gyeol</p></td></tr>
      <tr>
        <td style="padding:8px 32px 32px;">
          <h1 style="margin:0 0 16px;font-size:20px;color:${C.deep};font-weight:500;line-height:1.5;">${escape(name)}님, 연락해 주셔서 감사합니다.</h1>
          <p style="margin:0 0 14px;color:${C.ink};font-size:15px;line-height:1.75;">보내주신 이야기를 잘 받았습니다. 보통 1–2영업일 안에 회신드리겠습니다.</p>
  ...
  const { error } = await resend.batch.send([
    ...
    },
    {
      from: SENDER,
      to: email,
      subject: "[결] 연락해 주셔서 감사합니다",
      html: autoReplyHtml,
      text: `${name}님, 연락해 주셔서 감사합니다.\n보내주신 이야기를 잘 받았습니다. 보통 1–2영업일 안에 회신드리겠습니다.\n\n결 드림`,
    },
  ]);

  if (error) {
    console.error("[hub-contact] resend error", error);
    return NextResponse.json(
